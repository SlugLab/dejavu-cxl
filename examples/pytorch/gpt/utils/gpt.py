...
class GPT(nn.Module):
    def __init__(self,
        ...
        try:
            ...
            print(f"Exception when initializing: {e}")
        self.rank = dist.get_rank()
        self.device_count = torch.cuda.device_count()
        self.device =  (self.rank % (self.pipeline_para_size * self.tensor_para_size)) % self.device_count
        ...
        self.pipeline_para_rank = (self.rank % (self.pipeline_para_size * self.tensor_para_size)) // self.tensor_para_size
        print(f"from node {socket.gethostname()}, world_size is {world_size}, rank is {self.rank}, tensor_rank is {self.tensor_para_rank}, pipeline rank is {self.pipeline_para_rank}, device is {self.device}")

    def load(self, ckpt_path):
        is_load = self.weights.load(ckpt_path, tp_rank=self.tensor_para_rank,
                                    pipeline_para_rank=self.pipeline_para_rank)

        print("At load, call self.cuda")
        self.cuda()
    ...
    def forward(self,
        ...
        # Inputs to device
        if not start_ids.is_cuda:
            start_ids = start_ids.cuda(self.device)
        if not start_lengths.is_cuda:
