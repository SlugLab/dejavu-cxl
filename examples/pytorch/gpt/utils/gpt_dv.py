        ...
        if self.int8_mode != 0:
            for i in range(layer_num):
                ...
                if self.has_adapters:
                    ...
                    # Similar to above:
                    if self.int8_mode == 1:
                        self.w[12 * layer_num + i +
                               self.adapter_offset] = torch.empty(0).to(str_type_map[self.inference_data_type])
                        self.w[14 * layer_num + i +
                               self.adapter_offset] = torch.empty(0).to(str_type_map[self.inference_data_type])
                        self.w[16 * layer_num + i +
                               self.adapter_offset] = torch.empty(0).to(str_type_map[self.inference_data_type])
                        self.w[18 * layer_num + i +
                               self.adapter_offset] = torch.empty(0).to(str_type_map[self.inference_data_type])
        return True
...
class GPT(nn.Module):
    def __init__(self,
                 ...
                 vocab_size, start_id, end_id, layer_num, ckpt_path,
                 max_seq_len: int,
        ...
        self.prompt_world_size = prompt_world_size
        ...
        if token_world_size > 0:
            world_size = prompt_world_size + token_world_size
        else:
            world_size = dist.get_world_size() if dist.is_initialized() else 1
        self.rank = dist.get_rank() if dist.is_initialized() else torch_rank
        ...
        # This works either we have streaming or not
        if self.prompt_world_size > 0:
            base_rank = self.rank
            if self.rank >= self.prompt_world_size:
                base_rank -= self.prompt_world_size
            self.pipeline_para_rank = base_rank // self.tensor_para_size
        else:
        ...
        print(f"Rank {self.rank} After! Load to torch is {is_load}")
    ...
    def forward(self,
                start_ids: typing.List[torch.IntTensor],
                start_lengths: typing.List[torch.IntTensor],
                output_len: torch.IntTensor,
                beam_width: int = 1,
                restart: bool = False,
                ...
                finished: typing.Optional[torch.ByteTensor] = None,
                ubatch_ids: typing.Optional[typing.List[int]] = None,
                return_output_length: bool = False,
                return_cum_log_probs: int = 0):

        print(f"-------------------- In GPT-DV, forward")
        ...
        num_ubatches = len(start_ids)
        # Inputs to device
        for i in range(len(start_ids)):
            if not start_ids[i].is_cuda:
                start_ids[i] = start_ids[i].cuda(self.device)
        for i in range(len(start_lengths)):
            if not start_lengths[i].is_cuda:
                start_lengths[i] = start_lengths[i].cuda(self.device)

        # TODO(#11): adapt this
        start_ids = torch.cat(start_ids, 0)
        start_lengths = torch.cat(start_lengths, 0)

        if restart:
            restart_tensor = torch.ones(1, dtype=torch.bool)
        else:
            restart_tensor = None

        if streaming:
            streaming_tensor = torch.ones(1, dtype=torch.bool)
        else:
            streaming_tensor = None

        if swapping:
            swapping_tensor = torch.ones(1, dtype=torch.bool)
        else:
        ...
        outputs = self.model.forward(start_ids,
                                     ...
                                     output_len,
                                     ubatch_ids,  # optional, can be None
                                     return_cum_log_probs)  # optional, can be None
        if return_cum_log_probs == 0:
            output_ids, output_lengths, failure = outputs
        else:
            output_ids, output_lengths, output_cum_log_probs, failure = outputs
        if return_output_length:
            if return_cum_log_probs > 0:
                return output_ids, output_lengths, output_cum_log_probs, failure
            else:
                return output_ids, output_lengths, failure
        else:
            return output_ids, failure
...
class GptRuntimeModelParameters:
    ...
    def from_args(cls, args, config_reader, batch_size=None):
        ...
        return cls(
            ...
            repetition_penalty=(
                args.repetition_penalty or config_reader.getfloat(
