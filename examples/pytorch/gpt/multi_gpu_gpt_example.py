...
def main():
    ...
    parser.add_argument('--ckpt_path', type=str, default='../models/huggingface-models/c-model/gpt2-large/1-gpu/', help='path to the checkpoint file.')
    ...
    if args.sample_input_file:  # conditional case
        ...
        with open(args.sample_input_file, "r") as f:
            ...
            data = enc.decode(data_seq[:args.input_len])
            contexts = [data]*max_batch_size
    ...
    print(f"start_ids is {start_ids}")
    ...
    # Prepare model.
    if 'opt' in args.ckpt_path:
        print(ckpt_config)
        layernorm_eps = 1e-5
        layernorm_type = ckpt_config['gpt']['layernorm_type'] #'pre_layernorm' if ckpt_config['do_layer_norm_before'] else 'post_layernorm'
        activation_type = ckpt_config['gpt']['activation_type']
    ...
    if not args.use_gpt_decoder_ops:
        print("--------- ParallelGPT")
        if 'opt' in args.ckpt_path:
            print(layernorm_eps, layernorm_type, activation_type, has_post_decoder_layernorm)
    ...
    if not args.use_gpt_decoder_ops:
        def gpt_generate_fn(key_cache=None, val_cache=None):

            print("****************************** Inside gpt_generate_fn: ", output_len, return_output_length)
    ...
    for i in range(repeats):
        print(f"--------------------------- Start with request {i}")
        start = time.time()

        gen_outputs = gpt_generate_fn()
        key_cache = gen_outputs[1]
        value_cache = gen_outputs[2]
        ...
        print(f"Request {i} took {req_time} sec")
        #total_end = time.time()
        #print(f"******************** Total time is {total_end-total_start} sec")
    print(f"AVG: {np.mean(req_times)}, STD: {np.std(req_times)}")
    
    #res = f"gpt2,{args.output_len},{args.input_len},{args.max_batch_size},{np.mean(req_times)}, {np.std(req_times)}\n"
    #with open('ft/gpt2_res_limits_tokens.txt', 'a') as f:
    #    f.write(res)
    '''
    ...
    if rank == 0:
        ...
        outputs = []
        tokens_batch = tokens_batch[0].cpu().numpy()
