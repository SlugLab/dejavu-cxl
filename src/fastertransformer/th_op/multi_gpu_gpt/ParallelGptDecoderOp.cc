    ...
    int8_scales_(int8_scales)
{
    ...
    ftNcclInitialize(tensor_para_, pipeline_para_, pipeline_para_, tensor_para_size, pipeline_para_size, 0, 0, 0, 0);

    size_t local_num_layers = num_layers_ / pipeline_para_.world_size_;
    size_t first_layer      = local_num_layers * pipeline_para_.rank_;

    size_t start_layer = local_num_layers * pipeline_para_.rank_;
    size_t end_layer = local_num_layers * (pipeline_para_.rank_ + 1);
    if (pipeline_para_.rank_ == pipeline_para_.world_size_ - 1)
        end_layer = num_layers_;

    if (pipeline_para_.rank_ == pipeline_para_.world_size_ - 1)
        local_num_layers += (num_layers_ % pipeline_para_.world_size_);
