...
// since there are no barriers in NCCL, act as a barrier
                          ...
                          const bool with_mpi)
    {
        ...
        // For single GPU (no tensor or pipeline parallelism), skip NCCL initialization
        if (tensor_para_size == 1 && pipeline_para_size == 1)
        {
            printf("Single GPU setup detected (tensor_para_size=1, pipeline_para_size=1). Skipping NCCL initialization.\n");
            tensor_para.rank_ = 0;
            tensor_para.world_size_ = tensor_para_size;
            tensor_para.nccl_comm_ = nullptr;
            pipeline_para.rank_ = 0;
            pipeline_para.world_size_ = pipeline_para_size;
            pipeline_para.nccl_comm_ = nullptr;
            printf("NCCL skipped for single GPU: rank=%d world_size=%d tensor_para=%s pipeline_para=%s\n",
                   rank,
                   world_size,
        ...
        if (with_mpi)
        {
            ...
            if (!mpi_initialized)
            {
                // MPI is not initialized, fall back to NCCL-only mode
                printf("Warning: with_mpi=true but MPI is not initialized in ftNcclInitialize. Using NCCL-only mode.\n");
                // Set with_mpi to false and fall through to the else block
                // by using a goto or refactoring - for now, we'll duplicate the NCCL-only code
    ...
    void ftNcclCacheInitialize(NcclParam &cache_para, const int cache_para_size, const int torch_rank, const bool with_mpi)
    {
        ...
        // For single GPU, we don't need NCCL for cache streaming
        if (world_size == 1)
        {
            printf("Single GPU setup detected. Skipping NCCL cache initialization.\n");
            cache_para.world_size_ = world_size;
            cache_para.rank_ = torch_rank;
            cache_para.nccl_comm_ = nullptr;
            return;
        }
        ...
        if (with_mpi)
        {
            // Check if MPI is actually initialized
            int mpi_initialized = 0;
            MPICHECK(MPI_Initialized(&mpi_initialized));
            ...
            {
                // MPI is not initialized, fall back to NCCL-only mode
                printf("Warning: with_mpi=true but MPI is not initialized. Using NCCL-only mode.\n");
