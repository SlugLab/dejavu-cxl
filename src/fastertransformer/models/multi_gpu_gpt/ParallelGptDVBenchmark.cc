...
namespace fastertransformer
{
    ...
    ParallelGptDVBenchmark<T>::ParallelGptDVBenchmark(size_t max_batch_size,
                                                      ...
                                                      float shared_contexts_ratio) : BaseLayer(stream, cublas_wrapper, allocator, is_free_buffer_after_forward, cuda_device_prop, sparse),
                                                                                     head_num_(head_num),
    ...
    template <typename T>
    ParallelGptDVBenchmark<T>::ParallelGptDVBenchmark(ParallelGptDVBenchmark<T> const &gpt) : BaseLayer(gpt),
                                            ...
                                            const ParallelGptWeight<T> *gpt_weights)
    {
        ...
        FT_CHECK(input_tensors->find("output_seq_len") != input_tensors->end() && input_tensors->at("output_seq_len").shape.size() == 1);
        ...
        bool streaming = input_tensors->find("streaming") != input_tensors->end() ? input_tensors->at("streaming").getVal<bool>() : false;
        ...
        FT_CHECK_WITH_INFO(output_tensors->count("cum_log_probs") == 0 || output_tensors->at("cum_log_probs").size() == batch_size * beam_width,
        ...
        bool continue_gen = input_tensors->find("continue_gen") != input_tensors->end() ? input_tensors->at("continue_gen").getVal<bool>() : false;
        ...
        const int *prompt_learning_task_name_ids =
            input_tensors->count("prompt_learning_task_name_ids") ? input_tensors->at("prompt_learning_task_name_ids").getPtr<const int>() : nullptr;

        FT_CHECK_WITH_INFO(
            !(prompt_learning_task_name_ids != nullptr && (prompt_learning_type_ == PromptLearningType::no_prompt || prompt_learning_type_ == PromptLearningType::soft_prompt)),
        ...
        int valid_prompt_inputs = input_tensors->count("request_prompt_type") + input_tensors->count("request_prompt_lengths") + input_tensors->count("request_prompt_embedding");
        ...
        const uint32_t *input_lengths_h = input_tensors->count("input_lengths_h") ? input_tensors->at("input_lengths_h").getPtr<const uint32_t>() : nullptr;

        size_t max_input_without_prompt_length = max_context_len;
        if (use_request_p_prompt_embedding && input_lengths_h != nullptr && input_tensors->count("request_prompt_lengths_h"))
        {
        ...
        has_p_prompt_tuning_ =
            prompt_learning_task_name_ids != nullptr && prompt_learning_type_ == PromptLearningType::p_prompt_tuning || use_request_p_prompt_embedding;
        ...
        const size_t gen_len = reload ? input_tensors->at("output_seq_len").max<uint32_t>() + limit_len_offset + initial_step : input_tensors->at("output_seq_len").max<uint32_t>() + limit_len_offset;
        ...
        {
            // When the interactive generation mode is disabled, use max_seq_len to be safe
            // FIXME: This could be optimized to max_input_length + gen_len
            session_len = gpt_weights->getMaxSeqLen();
        }

        // session_len = session_len_ = 2000;  // REMOVED: Hardcoded debug value that causes buffer overflow
        session_len_ = session_len; // Set session_len_ from session_len
        FT_CHECK_WITH_INFO(
        ...
        // memory_len = memory_len_ = 2000;  // REMOVED: Hardcoded debug value that causes buffer overflow
        memory_len_ = memory_len; // Set memory_len_ from memory_len
        ...
        const bool is_return_context_cum_log_probs = input_tensors->count("is_return_context_cum_log_probs") > 0 && input_tensors->at("is_return_context_cum_log_probs").getVal<bool>();
        if (is_return_context_cum_log_probs)
        {
            FT_CHECK_WITH_INFO(output_tensors->count("cum_log_probs") && output_tensors->at("cum_log_probs").data != nullptr,
        ...
        if (streaming || reload)
        {
            total_cache_size_ = layers_per_pp_ * local_batch_size * beam_width * memory_len_ * hidden_units_ / tensor_para_.world_size_ * sizeof(T);
            token_cache_size_ =
        ...
        if (global_iteration_ > 0)
        {
            ...
            for (int ite = 0; ite < num_microbatches; ite++)
            {
                ...
                if (!done_[ite] && pipeline_para_.rank_ != pipeline_para_.world_size_ - 1 && pipeline_para_.world_size_ > 1)
                {
        ...
        for (int i = 0; i < num_microbatches; i++)
        {
            ...
            ubatch_step_end_[i] = *(input_tensors->at("output_seq_len").getPtr<uint32_t>() + i * local_batch_size) + limit_len_offset + initial_step;
        ...
        for (int ite = 0; ite < num_microbatches; ite++)
        {
            ...
            if (has_p_prompt_tuning_ || has_prefix_prompt_ || has_prefix_soft_prompt_ || max_input_length > 1)
            {
                ...
                invokeTileGptPromptInputs(
                    ...
                    use_request_p_prompt_embedding ? input_tensors->at("request_prompt_lengths").getPtr<const int>() : nullptr,
                    local_batch_size,
                    beam_width,
                    max_input_length,
                    stream_);
                ...
                else
                {
                    ...
                    pPromptTuningParam<T> prompt_param{
                        ...
                        use_request_p_prompt_embedding ? input_tensors->at("request_prompt_embedding").getPtr<T>() : nullptr};
                    ...
                    TensorMap decoder_input_tensors(
                        {{"decoder_input",
                          Tensor(MEMORY_GPU,
                                 ...
                                 gpt_variant_params_.has_pre_decoder_layernorm ? context_decoder_normed_input_buf_[ite] : context_decoder_input_buf_[ite])},
                         {"attention_mask",
                          Tensor(MEMORY_GPU,
                                 data_type,
            ...
            {
                FT_CHECK(prompt_learning_type_ == PromptLearningType::no_prompt && request_prompt_type == PromptLearningType::no_prompt);
                max_input_length++;
                PUSH_RANGE("decoding init");
                invokeDecodingInitialize(finished_buf_[ite],
                                         sequence_lengths_[ite],
        ...
        while (1)
        { // it will break when a microbatch finished
            ...
            for (uint ite = 0; ite < num_microbatches; ++ite)
            {
                ...
                if (ubatch_step_[ite] != ubatch_step_restart_[ite] && pipeline_para_.rank_ != pipeline_para_.world_size_ - 1 && pipeline_para_.world_size_ > 1)
                {
                ...
                if ((max_input_length <= 1) || (ubatch_step_[ite] > ubatch_step_start_[ite]) || continue_gen || reload)
                {
                    ...
                    std::unordered_map<std::string, Tensor> decoder_input_tensors(
                        {{"decoder_input",
                          Tensor(MEMORY_GPU,
                                 data_type,
                                 {local_batch_size * beam_width, hidden_units_},
                                 gpt_variant_params_.has_pre_decoder_layernorm ? decoder_normed_input_buf_[ite] : decoder_input_buf_[ite])},
                    ...
                    {
                        printf("[ERROR] Exception in gpt_decoder_->forward(): %s\n", e.what());
                        fflush(stdout);
                        throw;
                    }
                    fflush(stdout);
                ...
                if (!fill_caches_only && pipeline_para_.rank_ == pipeline_para_.world_size_ - 1)
                {
                    ...
                    T *decoder_output_final_buf = gpt_variant_params_.has_post_decoder_layernorm ? normed_decoder_output_buf_[ite] : decoder_output_buf_[ite];
                    ...
                    std::unordered_map<std::string, Tensor> dynamic_decode_output_tensors{
                        ...
                        {"cum_log_probs",
                         Tensor{MEMORY_GPU,
                                TYPE_FP32,
                                {local_batch_size * beam_width},
                                ((beam_width > 1) || (output_tensors->count("cum_log_probs") > 0)) ? cum_log_probs_[ite] : nullptr}},
                        {"output_log_probs",
                         Tensor{MEMORY_GPU,
                                TYPE_FP32,
                                {gen_len, local_batch_size, beam_width},
                                output_tensors->count("output_log_probs") > 0 && output_tensors->at("output_log_probs").data != nullptr ? output_log_probs_buf_[ite] : nullptr}},
                        {"parent_ids",
                ...
                if (ubatch_step_[ite] < ubatch_step_end_[ite] - 1 && pipeline_para_.world_size_ > 1 && pipeline_para_.rank_ == pipeline_para_.world_size_ - 1)
                {
                                                     ...
                                                     const size_t max_input_without_prompt_length)
    {
        ...
        for (int i = 0; i < num_microbatches; i++)
        {
            ...
            param.response_input_lengths = output_tensors->count("response_input_lengths") ? output_tensors->at("response_input_lengths").getPtr<int>() : nullptr;
