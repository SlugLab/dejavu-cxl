...
bool ParallelGptContextDecoder<T>::isLastLayerParallelId(uint l)
{
    ...
    if (pipeline_para_.rank_ == pipeline_para_.world_size_ - 1)
        end_layer = num_layer_;
    return l < num_layer_ && (l == end_layer - 1);
}
...
ParallelGptContextDecoder<T>::ParallelGptContextDecoder(size_t               max_batch_size,
                                                        ...
                                                        std::vector<void*>*  host_addr,
                                                        AttentionType        attention_type,
                                                        bool                 sparse,
                                                        int                  int8_mode,
                                                        std::shared_ptr<AbstractCustomComm> custom_all_reduce_comm,
                                                        int                                 enable_custom_all_reduce,
                                                        void**                              key_prompt_address,
                                                        void**                              value_prompt_address,
    ...
    key_prompt_address_(key_prompt_address),
    value_prompt_address_(value_prompt_address),
    copy_cache_(copy_cache),
...
template<typename T>
void ParallelGptContextDecoder<T>::flush_layer(int l, void* start_addr, size_t total_cache_size)
{

    // find receiver rank
    int recv_rank = pipeline_para_.world_size_ * tensor_para_.world_size_ + (l / layers_per_tp_)*tensor_para_.world_size_ + tensor_para_.rank_;
    recv_rank = std::min(recv_rank, cache_stream_para_->world_size_ - 1);  // in cases of uneven distribution of layers
    l         = l % layers_per_pp_;

    size_t offset = l * layer_prompt_size_;
    (*cache_manager_)->sync_streams();

    //printf("FLUSH LAYER %d to recv_rank %d\n", l, recv_rank);

    if (l % layers_per_tp_ == 0) {
#ifdef WITH_BOOST
        printf("SEND SLOT ID %d\n", (*cache_manager_)->prompt_slot_);
        (*cache_manager_)
            ->flush(&((*cache_manager_)->prompt_slot_), sizeof(int), recv_rank, NULL, 0);  // stream is not used here
#endif
    }

    // flush key cache
    char* key_cache_start = (char*)start_addr + offset;
    (*cache_manager_)
        ->flush((char*)key_cache_start, layer_prompt_size_, recv_rank, NULL, 0);  // stream is not used here
    // flush value cache
    char* value_cache_start = (char*)start_addr + total_cache_size + offset;
    (*cache_manager_)
        ->flush((char*)value_cache_start, layer_prompt_size_, recv_rank, NULL, 0);  // stream is not used here
}

template<typename T>
void ParallelGptContextDecoder<T>::copy_thread_func(int    ite,
                                                    int    prompt_size,
                                                    int    microbatch_size,
                                                    size_t total_cache_size)
{
    ...
    while (1) {
        ...
        for (uint l = 0; l < num_layer_; l++) {
            ...
            // copy locally
            copy_kv_cache_ubatch_layer(l % layers_per_pp_, ite, prompt_size, microbatch_size, total_cache_size);

            // flush to the proper destination
            size_t slot_offset = 2 * total_cache_size * (*cache_manager_)->prompt_slot_;
            flush_layer(l, (*host_address_)[0] + slot_offset, total_cache_size);
            layer_ready_[ite][l] = false;
        }
...
#ifndef WITH_BOOST
            printf("------------------------- [RANK %d] [UBATCH %d] Client %d, MarkComplete %d\n",
                   cache_stream_para_->rank_,
                   ite,
                   i,
                   (*cache_manager_)->prompt_slot_);
            dejavu_clients_[i]->MarkComplete((*cache_manager_)->prompt_slot_);
#endif
        }
...
template<typename T>
void ParallelGptContextDecoder<T>::copy_kv_cache_ubatch_layer(
    int layer, int microbatch_id, int prompt_size, int microbatch_size, size_t total_cache_size)
{

    int recv_rank = pipeline_para_.world_size_ * tensor_para_.world_size_ + (layer / layers_per_tp_)*tensor_para_.world_size_ + tensor_para_.rank_;
    layer         = layer % layers_per_pp_;
    std::vector<int> layers(1);
    layers[0] = layer;

    size_t slot_offset = 2 * total_cache_size * (*cache_manager_)->prompt_slot_;

    printf("[RANK %d] Inside copy_kv_cache_ubatch_layer, UBatch id: %d, Layer %d\n", pipeline_para_.rank_,
     microbatch_id, layer);

    // we assume 2 buffers for the cache
    StreamInfo task = {
        layers,
        microbatch_id % num_slots_,
        microbatch_size,
        0,
        prompt_size,
        true,
        recv_rank,
#if defined(NCCL_SEND)
        NULL,
        NULL
    };
#else
        (*host_address_)[0] + slot_offset,
...
template<typename T>
void ParallelGptContextDecoder<T>::set_cache_info(int beam_width, int max_input_len)
{
    beam_width_ = beam_width;
    prompt_len_ = max_input_len;
}

template<typename T>
void ParallelGptContextDecoder<T>::set_streaming_info(int                        num_microbatches,
                                                      int                        layers_per_tp,
                                                      int                        tp_per_pp,
                                                      std::vector<DejaVuClient*> dejavu_clients)
{
    num_microbatches_ = num_microbatches;
    layers_per_tp_    = layers_per_tp;
    restart           = (std::atomic_bool*)calloc(num_microbatches, sizeof(std::atomic_bool));
    layer_ready_      = (std::atomic_bool**)malloc(num_microbatches * sizeof(std::atomic_bool*));
    for (int i = 0; i < num_microbatches; i++) {
        layer_ready_[i]            = (std::atomic_bool*)calloc(num_layer_, sizeof(std::atomic_bool));
        stream_threads_created_.push_back(false);
    }
    dejavu_clients_ = dejavu_clients;
    tp_per_pp_      = tp_per_pp;
}
    ...
    const std::vector<ParallelGptDecoderLayerWeight<T>*>* gpt_decoder_layer_weight)
{
    ...
    const size_t batch_size = request_batch_size;
    ...
    // for copying
    cudaStream_t copy_stream1, copy_stream2;
    cudaStreamCreate(&copy_stream1);
    cudaStreamCreate(&copy_stream2);
    ...
    // TODO(#10): this used to take into account seq_len for computing the batch size, but leads to different batch
    // sizes compared to the one calculated in ParallelGpt.cc
    const size_t local_batch_size = decoder_input_tensor.shape[0];
...
#if defined(TEST_FAILURES) || defined(SEPERATE_PROMPT)
    for (int i = 0; i < num_microbatches_; i++) {
        if (!stream_threads_created_[i]) {
            stream_threads_.push_back(std::thread(&ParallelGptContextDecoder<T>::copy_thread_func,
                                                  this,
                                                  i,
                                                  prompt_len_,
                                                  local_batch_size,
                                                  total_cache_size));
            stream_threads_created_[i] = true;
        }
    }
#endif
    ...
    for (uint ite = 0; ite < iteration_num; ite++) {
        ...
        printf("Rank %d, Call for iteration %d, id is %u, batch_size is %lu, local_batch_size is %lu\n",
               pipeline_para_.rank_,
               ite,
               ubatch_id,
               batch_size,
        ...
        for (uint l = 0; l < num_layer_; l++) {
            ...
            // DIAGNOSTIC: Print hidden state norms at key layers
            if (l == 0 || l == 1 || l == num_layer_ / 2 || l == num_layer_ - 1) {
                // Copy a small slice to CPU to compute norm
                std::vector<float> h_vals(hidden_units_);
                // Get last position's hidden state: decoder_input[(h_token_num-1)*hidden_units_]
                cudaMemcpyAsync(h_vals.data(),
                               decoder_input + (h_token_num - 1) * hidden_units_,
                               hidden_units_ * sizeof(T),
                               cudaMemcpyDeviceToHost, stream_);
                cudaStreamSynchronize(stream_);
                // If T is half, we need to convert
                float norm = 0.0f;
                bool has_nan = false, has_inf = false;
                if constexpr (std::is_same<T, half>::value) {
                    const half* hp = reinterpret_cast<const half*>(h_vals.data());
                    std::vector<float> fvals(hidden_units_);
                    for (size_t i = 0; i < hidden_units_; i++) {
                        fvals[i] = __half2float(hp[i]);
                        norm += fvals[i] * fvals[i];
                        if (std::isnan(fvals[i])) has_nan = true;
                        if (std::isinf(fvals[i])) has_inf = true;
                    }
                    norm = sqrtf(norm);
                    printf("[FT][CTX_DIAG] Layer %u input (last pos): norm=%.4f, NaN=%d, Inf=%d, first8=[%.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f]\n",
                           l, norm, has_nan, has_inf,
                           fvals[0], fvals[1], fvals[2], fvals[3], fvals[4], fvals[5], fvals[6], fvals[7]);
                } else {
                    for (size_t i = 0; i < hidden_units_; i++) {
                        norm += h_vals[i] * h_vals[i];
                        if (std::isnan(h_vals[i])) has_nan = true;
                        if (std::isinf(h_vals[i])) has_inf = true;
                    }
                    norm = sqrtf(norm);
                    printf("[FT][CTX_DIAG] Layer %u input (last pos): norm=%.4f, NaN=%d, Inf=%d, first8=[%.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f]\n",
                           l, norm, has_nan, has_inf,
            ...
            if (layernorm_type_ == LayerNormType::pre_layernorm) {
                if (layer_weight->self_attn_layernorm_weights.beta == nullptr) {
                    ...
                    if (!has_adapters_ && attn_bias == nullptr) {
                        ...
                        // output = decoder_input + attn_out, norm_output = RMSNorm(output)
            ...
            POP_RANGE;
            // printf("Copy_cache_ %d, addr %p\n", copy_cache_, (*host_address_)[ubatch_id]);

#if defined(TEST_FAILURES) || defined(SEPERATE_PROMPT)
            if (copy_cache_)
                layer_ready_[ubatch_id][l] = true;
#else
#ifdef NCCL_SEND
            if (copy_cache_) {
#else
            if (copy_cache_ && (*host_address_)[ubatch_id] != NULL) {
#endif

                copy_kv_cache_ubatch_layer(l, ubatch_id, prompt_len_, local_batch_size, total_cache_size);

                auto                                      endl    = std::chrono::high_resolution_clock::now();
                std::chrono::duration<double, std::milli> ms_temp = endl - startl;
            }
#endif
        }
    }
