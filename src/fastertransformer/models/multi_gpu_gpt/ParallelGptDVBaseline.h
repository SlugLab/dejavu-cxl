...
static std::queue<int> avail_queue_;
static std::mutex      avail_mtx_;

static std::queue<int> written_queue_;
static std::mutex      written_mtx_;
static int             prompt_buffer_size_ = 3;

static std::vector<bool> ubatch_phase_;  // 0 for prompt, 1 for token
static std::vector<int>  ubatch_step_;
static std::vector<int>  ubatch_step_start_;
static std::vector<int>  ubatch_step_restart_;
static std::vector<int>  ubatch_step_end_;
static std::vector<int>  ubatch_global_id_;
static std::vector<bool> done_;

static DejaVuClient* dejavu_client_;

template<typename T>
class ParallelGptDVBaseline: public BaseLayer {
private:
    // meta data
    size_t               head_num_;
